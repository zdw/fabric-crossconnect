...
class TestKubernetesEvent(unittest.TestCase):
    def setUp(self):
        ...
        reload(
            synchronizers.new_base.mock_modelaccessor
    ...
    def test_process_event(self):
        with patch.object(
            FabricCrossconnectService.objects, "get_items"
        ) as fcservice_objects, patch.object(
            Service.objects, "get_items"
        ) as service_objects, patch.object(
            FabricCrossconnectServiceInstance, "save", autospec=True
        ) as fcsi_save:
            fcservice_objects.return_value = [self.fcservice]
            service_objects.return_value = [self.onos, self.fcservice]

            event_dict = {"status": "created", "labels": {"xos_service": "myonos"}}
            event = Mock()
